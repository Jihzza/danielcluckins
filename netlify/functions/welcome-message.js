...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Generate unique message_key
    const timestamp = Date.now();
    const randomString = Math.random().toString(36).substring(2, 15);
    const messageKey = `assistant-${session_id}-${timestamp}-${randomString}`;
    ...
    if (!existingFallback?.length) {
      ...
      // Generate unique message_key for fallback
      const fallbackTimestamp = Date.now();
      const fallbackRandomString = Math.random().toString(36).substring(2, 15);
      const fallbackMessageKey = `assistant-${session_id}-${fallbackTimestamp}-${fallbackRandomString}`;
      ...
      const { error: fallbackError } = await supabase
        ...
        .insert({
          ...
          role: 'assistant',
          content: fallback,
          message_key: fallbackMessageKey,
          created_at: new Date().toISOString(),
          is_welcome: true
        });
